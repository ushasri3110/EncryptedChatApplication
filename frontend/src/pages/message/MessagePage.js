...
function MessagePage({currentChat,reqUser}) {
    const jwt=localStorage.getItem('jwt');
    const {message}=useSelector(store=>store)
    const name=currentChat.isGroup?currentChat.chatName:reqUser?.userId !==currentChat.users[0]?.userId ? currentChat.users[0].fullName:currentChat.users[1].fullName;
    const image=currentChat.isGroup?currentChat.chatImage:reqUser?.userId!==currentChat.users[0]?.userId? currentChat.users[0].profilePic:currentChat.users[1].profilePic;
